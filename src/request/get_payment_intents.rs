...
impl<'a> GetPaymentIntentsRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<PaymentFlowsPaymentIntentList> {
        let mut r = self.http_client.client.get("/v1/payment_intents");
...
impl<'a> ::std::future::IntoFuture for GetPaymentIntentsRequest<'a> {
    type Output = httpclient::InMemoryResult<PaymentFlowsPaymentIntentList>;
