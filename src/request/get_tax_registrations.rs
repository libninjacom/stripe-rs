...
#[derive(Clone)]
pub struct GetTaxRegistrationsRequest<'a> {
...
impl<'a> GetTaxRegistrationsRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<
        TaxProductRegistrationsResourceTaxRegistrationList,
    > {
        let mut r = self.http_client.client.get("/v1/tax/registrations");
...
impl<'a> ::std::future::IntoFuture for GetTaxRegistrationsRequest<'a> {
