...
impl<'a> GetTransfersRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<TransferList> {
        let mut r = self.http_client.client.get("/v1/transfers");
        ...
        if let Some(ref unwrapped) = self.transfer_group {
            r = r.query("transfer_group", &unwrapped.to_string());
        }
...
impl<'a> ::std::future::IntoFuture for GetTransfersRequest<'a> {
    type Output = httpclient::InMemoryResult<TransferList>;
