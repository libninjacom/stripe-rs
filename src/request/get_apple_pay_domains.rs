...
impl<'a> GetApplePayDomainsRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<ApplePayDomainList> {
        let mut r = self.http_client.client.get("/v1/apple_pay/domains");
...
impl<'a> ::std::future::IntoFuture for GetApplePayDomainsRequest<'a> {
    type Output = httpclient::InMemoryResult<ApplePayDomainList>;
