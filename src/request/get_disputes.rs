...
impl<'a> GetDisputesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<DisputeList> {
        let mut r = self.http_client.client.get("/v1/disputes");
...
impl<'a> ::std::future::IntoFuture for GetDisputesRequest<'a> {
    type Output = httpclient::InMemoryResult<DisputeList>;
