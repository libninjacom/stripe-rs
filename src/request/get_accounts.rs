...
impl<'a> GetAccountsRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<AccountList> {
        let mut r = self.http_client.client.get("/v1/accounts");
...
impl<'a> ::std::future::IntoFuture for GetAccountsRequest<'a> {
    type Output = httpclient::InMemoryResult<AccountList>;
