...
impl<'a> GetTerminalLocationsRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<TerminalLocationLocationList> {
        let mut r = self.http_client.client.get("/v1/terminal/locations");
...
impl<'a> ::std::future::IntoFuture for GetTerminalLocationsRequest<'a> {
