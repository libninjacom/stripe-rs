...
impl<'a> GetTreasuryReceivedCreditsRequest<'a> {
    ...
    > {
        let mut r = self.http_client.client.get("/v1/treasury/received_credits");
        ...
        if let Some(ref unwrapped) = self.linked_flows {
            r = r.query("linked_flows", &unwrapped.to_string());
        }
...
impl<'a> ::std::future::IntoFuture for GetTreasuryReceivedCreditsRequest<'a> {
