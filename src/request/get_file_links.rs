...
impl<'a> GetFileLinksRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<FileResourceFileLinkList> {
        let mut r = self.http_client.client.get("/v1/file_links");
        ...
        if let Some(ref unwrapped) = self.expired {
            r = r.query("expired", &unwrapped.to_string());
        }
        if let Some(ref unwrapped) = self.file {
            r = r.query("file", &unwrapped.to_string());
        }
...
impl<'a> ::std::future::IntoFuture for GetFileLinksRequest<'a> {
    type Output = httpclient::InMemoryResult<FileResourceFileLinkList>;
