...
impl<'a> GetApplicationFeesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<PlatformEarningList> {
        let mut r = self.http_client.client.get("/v1/application_fees");
...
impl<'a> ::std::future::IntoFuture for GetApplicationFeesRequest<'a> {
