...
impl<'a> GetChargesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<ChargeList> {
        let mut r = self.http_client.client.get("/v1/charges");
...
impl<'a> ::std::future::IntoFuture for GetChargesRequest<'a> {
    type Output = httpclient::InMemoryResult<ChargeList>;
