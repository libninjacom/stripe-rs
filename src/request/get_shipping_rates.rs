...
impl<'a> GetShippingRatesRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<ShippingResourcesShippingRateList> {
        let mut r = self.http_client.client.get("/v1/shipping_rates");
...
impl<'a> ::std::future::IntoFuture for GetShippingRatesRequest<'a> {
    type Output = httpclient::InMemoryResult<ShippingResourcesShippingRateList>;
