...
impl<'a> GetTreasuryReceivedDebitsRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<TreasuryReceivedDebitsResourceReceivedDebitList> {
        let mut r = self.http_client.client.get("/v1/treasury/received_debits");
...
impl<'a> ::std::future::IntoFuture for GetTreasuryReceivedDebitsRequest<'a> {
