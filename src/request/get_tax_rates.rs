...
impl<'a> GetTaxRatesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<TaxRatesList> {
        let mut r = self.http_client.client.get("/v1/tax_rates");
        ...
        if let Some(ref unwrapped) = self.inclusive {
            r = r.query("inclusive", &unwrapped.to_string());
        }
...
impl<'a> ::std::future::IntoFuture for GetTaxRatesRequest<'a> {
    type Output = httpclient::InMemoryResult<TaxRatesList>;
