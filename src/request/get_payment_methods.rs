...
impl<'a> GetPaymentMethodsRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<PaymentFlowsPaymentMethodList> {
        let mut r = self.http_client.client.get("/v1/payment_methods");
...
impl<'a> ::std::future::IntoFuture for GetPaymentMethodsRequest<'a> {
    type Output = httpclient::InMemoryResult<PaymentFlowsPaymentMethodList>;
