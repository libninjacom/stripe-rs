...
impl<'a> GetIssuingDisputesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<IssuingDisputeList> {
        let mut r = self.http_client.client.get("/v1/issuing/disputes");
...
impl<'a> ::std::future::IntoFuture for GetIssuingDisputesRequest<'a> {
    type Output = httpclient::InMemoryResult<IssuingDisputeList>;
