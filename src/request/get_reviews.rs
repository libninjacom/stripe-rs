...
impl<'a> GetReviewsRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<RadarReviewList> {
        let mut r = self.http_client.client.get("/v1/reviews");
...
impl<'a> ::std::future::IntoFuture for GetReviewsRequest<'a> {
    type Output = httpclient::InMemoryResult<RadarReviewList>;
