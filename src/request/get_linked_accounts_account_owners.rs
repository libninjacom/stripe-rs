...
impl<'a> GetLinkedAccountsAccountOwnersRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<BankConnectionsResourceOwnerList> {
        let mut r = self
            .http_client
            .client
            .get(
        ...
        r = r.query("ownership", &self.ownership.to_string());
...
impl<'a> ::std::future::IntoFuture for GetLinkedAccountsAccountOwnersRequest<'a> {
