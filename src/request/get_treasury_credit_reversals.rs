...
impl<'a> GetTreasuryCreditReversalsRequest<'a> {
    ...
    > {
        let mut r = self.http_client.client.get("/v1/treasury/credit_reversals");
        ...
        if let Some(ref unwrapped) = self.received_credit {
            r = r.query("received_credit", &unwrapped.to_string());
        }
...
impl<'a> ::std::future::IntoFuture for GetTreasuryCreditReversalsRequest<'a> {
