...
impl<'a> GetFilesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<FileResourceFileList> {
        let mut r = self.http_client.client.get("/v1/files");
        ...
        if let Some(ref unwrapped) = self.purpose {
            r = r.query("purpose", &unwrapped.to_string());
        }
...
impl<'a> ::std::future::IntoFuture for GetFilesRequest<'a> {
