...
impl<'a> GetIdentityVerificationSessionsRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<GelatoVerificationSessionList> {
        let mut r = self.http_client.client.get("/v1/identity/verification_sessions");
...
impl<'a> ::std::future::IntoFuture for GetIdentityVerificationSessionsRequest<'a> {
