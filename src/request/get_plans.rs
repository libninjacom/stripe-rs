...
impl<'a> GetPlansRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<PlanList> {
        let mut r = self.http_client.client.get("/v1/plans");
...
impl<'a> ::std::future::IntoFuture for GetPlansRequest<'a> {
    type Output = httpclient::InMemoryResult<PlanList>;
