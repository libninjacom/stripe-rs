...
impl<'a> GetIssuingSettlementsRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<IssuingSettlementList> {
        let mut r = self.http_client.client.get("/v1/issuing/settlements");
...
impl<'a> ::std::future::IntoFuture for GetIssuingSettlementsRequest<'a> {
